...
def main_app():
    ...
    try:
        df = carregar_dados_desenhos(
            URL_PLANILHA_DESENHOS
        )
    ...
    if termo_input:
        ...
        resultados = buscar_desenho(
            df,
            termo_input
        )

        desenhos_encontrados = (
            resultados["DESENHO"]
            .dropna()
            .unique()
        )
        ...
        if len(desenhos_encontrados) > 0:
            ...
            for desenho in desenhos_encontrados:
                ...
                if len(revisoes_ordenadas) > 0:
                    ...
                    cols = st.columns(
                        len(revisoes_ordenadas)
                    )

                    ultima_revisao = (
                        revisoes_ordenadas[-1]
                    )
